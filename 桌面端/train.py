...
# 风格图片路径
STYLE_IMAGE_PATH = 'F:\大三上\自主学习\electron学习\electron-vue\imageTransform\src\static\images\style1.jpg'
# 生成图片的保存目录
OUTPUT_DIR = 'F:\大三上\自主学习\electron学习\electron-vue\imageTransform\src\static\output'
...
# 加载内容图片
content_image = utils.load_images(CONTENT_IMAGE_PATH)
# 风格图片
style_image = utils.load_images(STYLE_IMAGE_PATH)
...
def compute_content_loss(noise_content_features):
    ...
    for (noise_feature, factor), (target_feature, _) in zip(noise_content_features, target_content_features):
        layer_content_loss = _compute_content_loss(
            noise_feature, target_feature)
...
def _compute_style_loss(noise_feature, target_feature):
    ...
    style_loss = tf.reduce_sum(
        tf.square(noise_gram_matrix - style_gram_matrix))
    # 计算系数
    x = 4. * (M ** 2) * (N ** 2)
    return style_loss / x
